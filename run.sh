...
# Запуск unit-тестов
unit() {
  go test $(go list ./... | grep -e "git.mos.ru/buch-cloud/moscow-team-2.0/backend/$APP_NAME.git/")
}

# Запуск race-тестов
unit_race() {
  go test -race $(go list ./... | grep -e "git.mos.ru/buch-cloud/moscow-team-2.0/backend/$APP_NAME.git/")
}

# тест на покрытие
unit_coverage() {
    echo "run test coverage"
    go test $(go list ./... | grep -e "git.mos.ru/buch-cloud/moscow-team-2.0/backend/$APP_NAME.git/") -coverprofile=cover_profile.out.tmp $(go list ./internal/...)
